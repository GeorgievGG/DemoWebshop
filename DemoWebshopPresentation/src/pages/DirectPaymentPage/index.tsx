...
const DirectPaymentPage = () => {
  ...
  useEffect(() => {
      if (sessionState.PaymentCardToken) {
        fetch(`${process.env.REACT_APP_SERVER_URL}/api/Payment/Token/${sessionState.PaymentCardToken}`, {
          method: 'GET',
          headers: {
  ...
  const sendPayment = async (userInput: IPaymentCardData) => {
    if (shouldTokenizeCardData) {
      const tokenResponse = await fetch(`${process.env.REACT_APP_SERVER_URL}/api/Payment/Token`, {
      method: 'POST',
      headers: {
        'Content-type': 'application/json',
        'Authorization': `Bearer ${sessionState.Token}`
    ...
    const response = await fetch(`${process.env.REACT_APP_SERVER_URL}/api/Payment/ServerToServerPayment`, {
      method: 'POST',
      headers: {
        'Content-type': 'application/json',
        'Authorization': `Bearer ${sessionState.Token}`
